...
type GeneratedConfig struct {
	...
	// HighlightsLua is lua/{namespace}/core/highlights.lua (custom highlight groups)
	HighlightsLua string
...
func (g *Generator) WriteToDirectory(cfg *CoreConfig, plugins []*plugin.Plugin, dir string) error {
	...
	// Only write highlights.lua if there are custom highlights
	if generated.HighlightsLua != "" {
		files[filepath.Join(dir, "lua", ns, "core", "highlights.lua")] = generated.HighlightsLua
	}
...
func (g *Generator) generateCoreInitLua(cfg *CoreConfig, namespace string) string {
	var lua strings.Builder
	lua.WriteString(fmt.Sprintf("require(\"%s.core.options\")\n", namespace))
	lua.WriteString(fmt.Sprintf("require(\"%s.core.keymaps\")\n", namespace))
	lua.WriteString(fmt.Sprintf("require(\"%s.core.autocmds\")\n", namespace))
	if len(cfg.CustomHighlights) > 0 {
		lua.WriteString(fmt.Sprintf("require(\"%s.core.highlights\")\n", namespace))
	}
	return lua.String()
}
...
// generateHighlightsLua creates the core/highlights.lua file with custom highlight groups.
// Each highlight group generates a vim.api.nvim_set_hl(0, ...) call.
func (g *Generator) generateHighlightsLua(cfg *CoreConfig) string {
	if len(cfg.CustomHighlights) == 0 {
		return ""
	}

	var lua strings.Builder
	lua.WriteString("-- Custom highlight groups\n")
	lua.WriteString("-- Generated by DevOpsMaestro\n\n")

	// Sort group names for deterministic output
	names := make([]string, 0, len(cfg.CustomHighlights))
	for name := range cfg.CustomHighlights {
		names = append(names, name)
	}
	sort.Strings(names)

	for _, name := range names {
		hl := cfg.CustomHighlights[name]
		g.writeHighlightGroup(&lua, name, &hl)
	}

	return lua.String()
}

// writeHighlightGroup writes a single vim.api.nvim_set_hl() call.
func (g *Generator) writeHighlightGroup(lua *strings.Builder, name string, hl *HighlightGroup) {
	if hl.Link != "" {
		// Link to another highlight group
		lua.WriteString(fmt.Sprintf("vim.api.nvim_set_hl(0, \"%s\", { link = \"%s\" })\n",
			escapeString(name), escapeString(hl.Link)))
		return
	}
	...
	if hl.Fg != "" {
		attrs = append(attrs, fmt.Sprintf("fg = \"%s\"", escapeString(hl.Fg)))
	}
	if hl.Bg != "" {
		attrs = append(attrs, fmt.Sprintf("bg = \"%s\"", escapeString(hl.Bg)))
	}
	if hl.Sp != "" {
		attrs = append(attrs, fmt.Sprintf("sp = \"%s\"", escapeString(hl.Sp)))
	}
	if hl.Bold {
	...
	lua.WriteString(fmt.Sprintf("vim.api.nvim_set_hl(0, \"%s\", { %s })\n",
		escapeString(name), strings.Join(attrs, ", ")))
}
